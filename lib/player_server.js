...
var MAX_LABEL_NAME_LEN = 512;
var MAX_COLOR_LEN = 16;
...
var GROOVEBASIN_PROTOCOL_VERSION = "0.0.1";
...
// When editing this structure, and this entire file in general, it is your
// duty to also make the relevant changes to doc/protocol.md.
var actions = {
  'approve': {
    permission: 'admin',
  ...
  'subscribe': {
    ...
    fn: function(self, client, args) {
      ...
      if (!client.transport.userHasPerm(subscription.perm)) {
  ...
  'playlistCreate': {
    ...
    args: objectArg({
      id: stringArg(UUID_LEN),
      name: stringArg(MAX_LABEL_NAME_LEN),
    }),
    fn: function(self, client, args) {
  ...
  'labelCreate': {
    ...
    fn: function(self, client, args) {
      self.labelCreate(client, args.id, args.name);
    },
  },
  'labelRename': {
    permission: 'playlist',
    ...
    fn: function(self, client, args) {
      self.labelRename(client, args.id, args.name);
    },
  },
  'labelColorUpdate': {
    permission: 'playlist',
    args: objectArg({
      id: stringArg(UUID_LEN),
      color: stringArg(MAX_COLOR_LEN),
    }),
    fn: function(self, client, args) {
      self.labelColorUpdate(client, args.id, args.color);
    },
  },
  'labelDelete': {
    permission: 'playlist',
    args: arrayArg(stringArg(UUID_LEN), MAX_KEY_COUNT),
    fn: function(self, client, ids) {
      self.labelDelete(client, ids);
    },
  },
  'labelAdd': {
    permission: 'playlist',
    args: dictArg(arrayArg(stringArg(UUID_LEN), MAX_KEY_COUNT), MAX_KEY_COUNT, UUID_LEN),
    fn: function(self, client, additions) {
      self.labelAdd(client, additions);
    },
  },
  'labelRemove': {
    permission: 'playlist',
    args: dictArg(arrayArg(stringArg(UUID_LEN), MAX_KEY_COUNT), MAX_KEY_COUNT, UUID_LEN),
    fn: function(self, client, removals) {
      self.labelRemove(client, removals);
    },
  },
  'lastFmGetSession': {
    permission: 'read',
...
PlayerServer.prototype.initialize = function() {
  ...
  var onLabelUpdate = addSubscription('labels', serializeLabels);
  self.player.on('labelCreate', onLabelUpdate);
  self.player.on('labelRename', onLabelUpdate);
  self.player.on('labelColorUpdate', onLabelUpdate);
  self.player.on('labelDelete', onLabelUpdate);
  ...
  var onStreamersUpdate = addSubscription('anonStreamers', serializeStreamers);
  self.player.on('streamerConnect', onStreamersUpdate);
  self.player.on('streamerDisconnect', onStreamersUpdate);

  // Ideally this would happen when the system time changes. For now we will
  // trigger it every 30 seconds.
  ...
  addSubscription('protocolMetadata', getProtocolMetadata);

  addSubscription('streamEndpoint', getStreamEndpoint);
  ...
  function getQueueLibraryInfo() {
    ...
    for (var itemId in self.player.playlist) {
      ...
      table[track.key] = Player.trackWithoutIndex('clientVisible', track);
    }
    return table;
  }
  ...
  function serializeLibrary() {
    ...
    for (var key in self.player.libraryIndex.trackTable) {
      ...
      table[key] = Player.trackWithoutIndex('clientVisible', track);
    }
    return table;
  }
  ...
  function serializePlaylists() {
    ...
    for (var key in self.player.playlists) {
      ...
      var outItems = {};
      for (var itemId in pl.items) {
        var plItem = pl.items[itemId];
        outItems[itemId] = {
          key: plItem.key,
  ...
  function serializeLabels() {
    var o = {};
    for (var key in self.player.libraryIndex.labelTable) {
      var labelEntry = self.player.libraryIndex.labelTable[key];
      o[key] = {
        name: labelEntry.name,
        color: labelEntry.color,
      };
    }
    console.log("labels:", o);
    return o;
  }
  ...
  function getProtocolMetadata() {
    ...
    var infoSet = {};
    for (var infoName in self.subscriptions) {
      infoSet[infoName] = true;
    }
    return {
      version: GROOVEBASIN_PROTOCOL_VERSION,
      actions: actionSet,
      information: infoSet,
      httpActions: {
        "GET /library/": true,
        "GET /library/[folder]/": true,
        "GET /library/[songFilePath]": true,
        "GET /download/keys": true,
        "GET /[streamEndpoint]": true,
        "POST /upload": true,
      }
    };
  }
...
PlayerServer.prototype.createGuestUser = function(prefix, transport) {
...
PlayerServer.prototype.createClient = function(transport, prefix) {
  ...
  var client = {
    ...
    user: self.createGuestUser(prefix, transport),
    streaming: false,
  };
  ...
  function onMessage(name, args) {
    ...
    var errMsg;
    if (perm != null && !client.transport.userHasPerm(perm)) {
...
PlayerServer.prototype.login = function(client, username, password) {
  if (client.transport.username) {
    client.user.name = client.transport.username;
  }
...
PlayerServer.prototype.sendUserMessage = function(client) {
  client.transport.sendMessage('user', {
    ...
    perms: client.transport.getUserPerms(),
...
PlayerServer.prototype.addEvent = function(options) {
  ...
  var subCount = options.subCount;

  var lastEvent = this.eventsInOrder[this.eventsInOrder.length - 1];
  if (lastEvent && ('string' != (typeof lastEvent.sortKey))) {
...
PlayerServer.prototype.labelCreate = function(client, id, name) {
  if (name.length === 0) {
    log.warn("empty label name");
    return;
  }
  this.addEvent({
    user: client.user,
  ...
  this.player.labelCreate(id, name);
};

PlayerServer.prototype.labelRename = function(client, labelId, name) {
  if (name.length === 0) {
    log.warn("empty label name");
    return;
  }
  var label = this.player.libraryIndex.labelTable[labelId];
  if (!label) {
    log.warn("invalid label id");
    return;
  }
  ...
  this.player.labelRename(labelId, name);
};

PlayerServer.prototype.labelColorUpdate = function(client, labelId, color) {
  if (color.length === 0) {
    log.warn("empty color");
    return;
  }
  var label = this.player.libraryIndex.labelTable[labelId];
  if (!label) {
  ...
  this.addEvent({
    user: client.user,
    type: 'labelColorUpdate',
    labelId: labelId,
    text: oldColor,
  });
  this.player.labelColorUpdate(labelId, color);
};

PlayerServer.prototype.labelDelete = function(client, labelIds) {
  for (var i = 0; i < labelIds.length; i += 1) {
  ...
  this.player.labelDelete(labelIds);
};

PlayerServer.prototype.labelAdd = function(client, additions) {
  var count = keyCount(additions);
  if (count === 0) return;
  var key = firstKeyInObject(additions);
  this.addEvent({
    user: client.user,
    type: 'labelAdd',
    trackKey: key,
    pos: count,
    labelId: additions[key][0],
    subCount: additions[key].length,
  });
  this.player.labelAdd(additions);
};

PlayerServer.prototype.labelRemove = function(client, removals) {
  var count = keyCount(removals);
  if (count === 0) return;
  var key = firstKeyInObject(removals);
  this.addEvent({
    user: client.user,
    type: 'labelRemove',
    trackKey: key,
    pos: count,
    labelId: removals[key][0],
    subCount: removals[key].length,
  });
  this.player.labelRemove(removals);
};
...
PlayerServer.prototype.insertTracks = function(client, index, keys, tagAsRandom) {
  ...
  return this.player.insertTracks(index, keys, tagAsRandom);
};
...
PlayerServer.dumpUsers = function(db) {
    dbIterate(db, USERS_KEY_PREFIX, processOne, allDone);
