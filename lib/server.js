...
var STREAM_ENDPOINT = "stream.mp3";
...
function initializeStream(app, player, hasPermRead) {
  app.get('/' + STREAM_ENDPOINT, hasPermRead, function(req, resp, next) {
...
function createHasPermMiddleware(playerServer, permName) {
  return function(req, resp, next) {
    ...
    var hasPermission = client && (client.transport.userHasPerm(permName));
    if (hasPermission) {
      req.client = client;
      resp.client = client;
      next();
...
function startServer(app, db, player, config, sslEnabled) {
  ...
  function attachWebSocketServer() {
    ...
    wss.on('connection', function(ws, request) {
      playerServer.createClient(new WebSocketApiClient(ws, request), "Guest");
    });
    ...
    if (mpdPort == null || mpdHost == null) {
      ...
      protocolServer.listen(mpdPort, mpdHost, function() {
        log.info("MPD Protocol listening at " +
          mpdHost + ":" + mpdPort);
      });
    }
  }
